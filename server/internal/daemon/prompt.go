...
func BuildPrompt(task Task, provider string) string {
	if task.ChannelNLMessage != "" {
		return buildChannelNLPrompt(task)
	}
...
// buildChannelNLPrompt constructs the prompt for a channel NL-query task.
// The user sent a natural-language workspace-management message via Slack or
// Telegram and no cloud AI provider key is configured. The agent runs in
// single-turn conversational mode and its final stdout is the channel reply.
func buildChannelNLPrompt(task Task) string {
	var b strings.Builder
	b.WriteString("You are a Multica workspace assistant responding to a message from a messaging channel (Slack or Telegram).\n\n")
	b.WriteString("Use the `multica` CLI to fulfill the request. When listing issues, be concise — show title, status, priority, and ID. Print only the reply text as your final output — it will be sent directly to the user in the channel.\n\n")
	fmt.Fprintf(&b, "User message:\n%s\n", task.ChannelNLMessage)
	return b.String()
}
